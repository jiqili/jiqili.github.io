...
# 解决办法
- 专门搞一个csrf token登录后服务端返回给客户端，客户端存起来后每次请求时把csrf token作为参数传过去,服务端可以搞个中间件来判断token是否合法。注意token不要存在cookie里，不然csrf攻击页面又自动带上cookie了。不过这样有些麻烦，尤其是对于第三方请求。
- 指定`SameSite`为`strict`或者`Referer`里判断哪个路由发出的请求，或者`Origin`判断哪个域名发出的请求。有时候`Origin`和`Referer`字段可能没有，比如地址栏里直接输入地址访问。

# 思考
1. 为什么浏览器要自动携带cookie？

因为cookie是为了解决http无状态的缺点，否则用户每次请求都要跳到登陆页面来进行合法判定。有了自动带上cookie后，就可以创造一种连贯的，有状态的用户体验。

2. 那为什么要用会带来CSRF风险的cookie存登录凭证呢？localStorage能否替代cookie？
